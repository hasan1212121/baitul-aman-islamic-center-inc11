  ...
  return (
    <main style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
      ...
      <header className="glass-panel" style={{ textAlign: 'center', marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '3rem', color: 'var(--accent-color)', marginBottom: '0.5rem', fontFamily: 'serif' }}>{t.heading}</h1>
        <p style={{ fontSize: '1.2rem', color: 'var(--text-muted)' }}>{t.address}</p>
      ...
      <footer className="footer" style={{ marginTop: '3rem' }}>
         ...
         <p style={{ fontSize: '0.85rem', marginTop: '1rem', color: 'var(--text-muted)' }}>{t.rights}</p>
      </footer>
    </main>
  );
}
